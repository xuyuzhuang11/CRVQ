...
class QuantizedWeight(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        out_group_size: int,
        num_codebooks: int,
        nbits_per_codebook: List = [8],
        codebook_value_nbits: int = 16,
        codebook_value_num_groups: int = 1,
        scale_nbits: int = 0,
        straight_through_gradient: Optional[bool] = None,
        multibook_ratio: float = 1.0,
        shuffle_rule: Literal[1, 2, 3] = 3,
        **init_kwargs,
    ):
        super().__init__()
        # print(reference_weight.dtype)
        ...
        self.codebook_size = codebook_size = [2 ** persize for persize in nbits_per_codebook]
        ...
        with torch.no_grad():
            ...
            if scale_nbits > 0:
                ...
                # print(scales.shape) [4096, 1, 1, 1] float
        ...
        self.multibook_ratio = multibook_ratio
        self.in_channel_bound = int((self.in_features * multibook_ratio // in_group_size) * in_group_size)
        self.group_bound = int(self.in_features * multibook_ratio // in_group_size)
        ...
        weight_residue = (
            reference_weight.clone().reshape(num_out_groups, out_group_size, num_in_groups, in_group_size)
            .swapaxes(-3, -2)
        ...
        codebook_i, _, _ = fit_kmeans(
            ...
            k=codebook_size[0],
            max_iter=1000,
            devices=[weight_residue.device],
            # **init_kwargs
        )
        _, reconstructed_weight_i = find_nearest_cluster(weight_residue, codebook_i, devices=[weight_residue.device])
        weight_residue -= reconstructed_weight_i
        weight_residue = (
            weight_residue.reshape(num_out_groups, num_in_groups, out_group_size, in_group_size)
            .swapaxes(-3, -2)
            .reshape(reference_weight.shape)
        )
        diag = torch.diag(XTX).view(1, -1)
        self.sorted_indices = _return_shuffle_index((weight_residue ** 2) * diag, reference_weight, self.in_channel_bound, self.shuffle_rule)
        
        weight_for_init = weight_for_init[:, self.sorted_indices]
        ...
        codes, codebooks = init_aq_kmeans(
            ...
            multibook_ratio=self.multibook_ratio,
            **init_kwargs,
        )

        self.codebooks = nn.ParameterList([
            nn.Parameter(codebook, requires_grad=True)
            for codebook in codebooks
        ]) # List of [codebook_size, out_group_size, in_group_size]
        self.codes = nn.ParameterList([
            nn.Parameter(code.to(torch.int32), requires_grad=False)
            for code in codes
        ]) # List of [num_out_groups, num_in_groups]
        
    def to(self, *args, **kwargs):
        super().to(*args, **kwargs)
        self.codebooks = [codebook.to(*args, **kwargs) for codebook in self.codebooks]
        return self
    
    def named_custom_parameters(self):
        params = {}
    ...
    def forward(self, selection: Union[slice, ellipsis, torch.Tensor] = ...):
        ...
        """
        weight = _dequantize_weight(self.codes, self.get_codebooks(), self.get_scales(), self.sorted_indices, self.multibook_ratio)
        return weight
    ...
    def estimate_nbits_per_parameter(self) -> float:
        ...
        group_bound = int(self.in_features * self.multibook_ratio // self.in_group_size)
        matrix_store = num_parameters // group_size * self.nbits_per_codebook[0]
        for nbits in self.nbits_per_codebook[1:]:
            matrix_store += num_out_groups * group_bound * nbits

        codebooks_store = sum(self.codebook_size) * group_size * self.codebook_value_nbits
        if self.codebook_value_nbits < 16:
            assert False, 'quantized codebook is not considered'
            codebooks_store += (
...
def beam_search_optimal_codes(
    *,
    ...
    prev_codes: List[torch.IntTensor],
    scales: Optional[torch.Tensor],
    groupbound: int,
    sorted_indices: torch.Tensor,
    multibook_ratio: float,
    beam_size: int,
    ...
    :note: if XTX is divided by dataset size, this function will return *mean* squared error
    :param reference_weight: original weight matrix that is being quantized, shape: [out_features, in_features]
    :param codebooks: look-up tables of codes, shape: List[codebook_size, out_group_size, in_group_size]
    :param prev_codes: previous-best integer weight codes, shape: List[num_out_groups, num_in_groups]
    ...
    - the codes are of shape [out_features / out_group_size, in_features / in_group_size]
    ...
    """
    num_out_groups, num_in_groups, num_codebooks = prev_codes[0].shape[0], prev_codes[0].shape[1], len(prev_codes)
    num_codebooks, codebook_size, out_group_size, in_group_size = len(codebooks), codebooks[0].shape[0], codebooks[0].shape[1], codebooks[0].shape[2]
    in_features = num_in_groups * in_group_size
    out_features = num_out_groups * out_group_size
    assert reference_weight.shape == (out_features, in_features)
    prev_weight = _dequantize_weight(prev_codes, codebooks, scales, sorted_indices, multibook_ratio)
    # initialize all beam codes as previous codes - so they can be updated during beam search
    beam_codes = [prev_code.unsqueeze(0) for prev_code in prev_codes]
    # beam_codes shape: List [current beam_size, num_out_groups, num_in_groups], initial beam_size = 1
    ...
    for input_group_index in _make_range(num_in_groups):
        for codebook_index in _make_range(num_codebooks):
            ...
            beam_codes, beam_weights, beam_losses = _beam_search_select_best(
                ...
                beam_size=beam_size,
                sorted_indices=sorted_indices,
                multibook_ratio=multibook_ratio
            )
    ...
    res_beam_codes = nn.ParameterList([nn.Parameter(code[0].to(torch.int32), requires_grad=False) for code in beam_codes])
    # return beam_codes[0]
    return res_beam_codes


# @maybe_script
def _dequantize_weight(
    codes: List[torch.Tensor], codebooks: List[torch.Tensor], scales: Optional[torch.Tensor], sorted_indice: torch.Tensor, multibook_ratio: float = 1.0,
) -> torch.Tensor:
    """
    ...
    """
    num_out_groups, num_in_groups, num_codebooks = codes[0].shape[0], codes[0].shape[1], len(codes)
    # num_codebooks, codebook_size, out_group_size, in_group_size = codebooks.shape
    codebook_size, out_group_size, in_group_size = codebooks[0].shape[0], codebooks[0].shape[1], codebooks[0].shape[2]
    out_features = num_out_groups * out_group_size
    in_features = num_in_groups * in_group_size
    group_bound = int(in_features * multibook_ratio // in_group_size)
    # List of [codebook_size, out_group_size, in_group_size]
    # List of [num_out_groups, num_in_groups]
    reconstructed_weight_groupwise = F.embedding(
        codes[0].flatten(0, -1).clone(), codebooks[0].flatten(-2, -1)
    ).view([num_out_groups, num_in_groups, out_group_size, in_group_size])

    # list(codes.shape[:-3]) is omitted here
    for i in range(1, len(codebooks), 1):

        reconstructed_weight_group_small = F.embedding(
            codes[i].flatten(0, -1).clone(), codebooks[i].flatten(-2, -1)
        ).view(num_out_groups, group_bound, out_group_size, in_group_size)
        reconstructed_weight_groupwise[:, :group_bound, :, :] += reconstructed_weight_group_small

    reconstructed_weight_groupwise = reconstructed_weight_groupwise.swapaxes(-3, -2).reshape([out_features, in_features])
    reverse_indices = torch.argsort(sorted_indice)
    reconstructed_weight_groupwise = reconstructed_weight_groupwise[:, reverse_indices]
    if scales is not None:
        scales = scales.squeeze().unsqueeze(1)
        reconstructed_weight_groupwise = reconstructed_weight_groupwise * scales
    return reconstructed_weight_groupwise
...
def _beam_search_squared_errors(
    ...
    :param beam_codes: a tensor with best weight codes, shape: List[beam_size, num_out_groups, num_in_groups]
    ...
    """
    num_codebooks, out_group_size, in_group_size = len(codebooks), codebooks[0].shape[1], codebooks[0].shape[2]
    codebook_size = [codebook.shape[0] for codebook in codebooks]
    beam_size, num_out_groups, num_in_groups, num_codebooks = beam_codes[0].shape[0], beam_codes[0].shape[1], beam_codes[0].shape[2], len(beam_codes)
    out_features = num_out_groups * out_group_size

    input_group_slice = slice(input_group_index * in_group_size, (input_group_index + 1) * in_group_size)

    prev_codes_part = beam_codes[codebook_index][:, :, input_group_index]  # [beam_size, num_out_groups]
    ...
    prev_weight_part = prev_part_dequantized    # beam_size, out_features, in_group_size
    if scales is not None:
        prev_weight_part = (
    ...
    input_group_slice = sorted_indices[input_group_slice]
    ...
    # print(cand_weights.flatten(0, 1).shape, XTX[input_group_slice, :][:, input_group_slice].shape)
    XnewBkC_norms_sq = torch.bmm(
        (cand_weights.flatten(0, 1) @ XTX[input_group_slice, :][:, input_group_slice]).view(
            codebook_size[codebook_index], 1, out_group_size * in_group_size
        ),
        cand_weights.view(codebook_size[codebook_index], out_group_size * in_group_size, 1),
    ).reshape(
        codebook_size[codebook_index], 1
    ...
    for beam_id in range(beam_size):
        dot_products = (
            torch.einsum(
                "mg,og->mo",
                cand_weights.reshape(codebook_size[codebook_index], out_group_size * in_group_size),
            ...
            .view(codebook_size[codebook_index], num_out_groups)
        )
        if scales is not None:
...
def _beam_search_select_best(
    ...
    :param beam_codes: a tensor with best weight codes, shape: List[beam_size, num_out_groups, num_in_groups]
    :param beam_weights: a tensor with de-quantized beam_codes, shape: [beam_size, out_features, in_features]
    :param codebooks: a tensor with look-up tables of codes, shape: List[codebook_size, out_group_size, in_group_size]
    :param scales: weight will be multiplied by this factor, [num_out_groups, num_in_groups, 1, 1]
    ...
    num_out_groups, num_in_groups, num_codebooks = beam_codes[0].shape[1], beam_codes[0].shape[2], len(beam_codes)
    ...
    new_beam_codes = [
        torch.full(
            size=(len(best_hypo_codes), num_out_groups, beam_codes[i].shape[2]),
            fill_value=-1,
            dtype=beam_codes[0].dtype,
            device=device,
        )
        for i in range(num_codebooks)
    ] # List of [beam_size, num_out_groups, num_in_groups]
    ...
    for beam_index in range(len(best_hypo_codes)):
        for codebook_idx in range(num_codebooks):
            new_beam_codes[codebook_idx][beam_index, :, ...] = beam_codes[codebook_idx][best_hypo_source_ids[beam_index, :], arange_out_groups, ...]
            # new_beam_codes[beam_index, :, ...] = beam_codes[best_hypo_source_ids[beam_index, :], arange_out_groups, ...]
        new_beam_codes[codebook_index][beam_index, :, input_group_index] = best_hypo_codes[beam_index, :]
        beamed_codes = [new_beam_codes[codebook_idx][beam_index, ...] for codebook_idx in range(num_codebooks)]
        new_beam_weights[beam_index, :, :] = _dequantize_weight(beamed_codes, codebooks, scales, sorted_indices, multibook_ratio)
...
def _return_shuffle_index(SpQR_tensor: torch.Tensor, ref_tensor: torch.Tensor, in_channel_bound, shuffle_rule: Literal[1, 2, 3] = 3) -> torch.Tensor:

    print(f'----------------------------------shuffle_rule：{shuffle_rule}--------------------------------------\n')
    # sorting randomly
    if shuffle_rule == 1: 
        column_means = ref_tensor.abs().mean(dim=0)
        generator = torch.Generator()
        generator.manual_seed(1234)  
        sorted_indices = torch.randperm(len(column_means), generator=generator)

    # sorting by W-only
    if shuffle_rule == 2: 
        column_means = ref_tensor.abs().mean(dim=0)
        sorted_indices = torch.argsort(column_means, descending=True)

    # sorting by W-A
    if shuffle_rule == 3: 
        column_means = SpQR_tensor.abs().mean(dim=0)
        sorted_indices = torch.argsort(column_means, descending=True)
...
def init_aq_kmeans(
    ...
    **kwargs,
    ...
    group_bound = int(in_features * multibook_ratio // in_group_size)
    # print(f'{in_features=}')
    # print(f'{multibook_ratio=}')
    # print(f'{in_group_size=}')
    weight_residue = reference_weight.clone()
    codebooks = []
    codes = []

    if max_points_per_centroid is not None:
        assert False, 'max_points_per_centroid is not suitable for multibook now'
        print("Clustering:", max_points_per_centroid * codebook_size, "points from", weight_residue.shape[0])

    for i in trange(num_codebooks, desc="initializing with kmeans") if verbose else range(num_codebooks):
        if i == 0:
            weight_residue = (
                weight_residue.reshape(num_out_groups, out_group_size, num_in_groups, in_group_size)
                .swapaxes(-3, -2)
                .reshape(num_out_groups * num_in_groups, out_group_size * in_group_size)
            )
            weight_fit = weight_residue.clone()

        elif i == 1:
            weight_residue = (
                weight_residue.reshape(num_out_groups, num_in_groups, out_group_size, in_group_size)[:, :group_bound, :, :]
                .reshape(num_out_groups * group_bound, out_group_size * in_group_size)
            )
            weight_fit = weight_residue
        else:
            pass
        ...
        if use_faiss:
            ...
            codebook_i, _, _ = fit_kmeans(
                weight_fit if chosen_ids is None else weight_fit[chosen_ids, :],
                k=codebook_size[i],
                max_iter=max_iter,
                devices=devices,
                **kwargs,
        ...
        codes_i = codes_i.reshape(num_out_groups, num_in_groups if i == 0 else group_bound)

        codebook_i = codebook_i.reshape(codebook_size[i], out_group_size, in_group_size)
