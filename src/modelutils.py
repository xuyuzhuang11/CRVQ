    ...
    with suspend_nn_inits():
        ...
        if load_quantized:
            ...
            if model.config.model_type == "opt":
                model = load_OPT_model(model, load_quantized)
            else:
                model = load_quantized_model(model, load_quantized)

            if device_map == "auto" and model.config.model_type != "opt":
...
def load_OPT_model(model, load_path):
    """Load quantized model"""

    for layer_index in range(len(model.model.decoder.layers)):
        model.model.decoder.layers[layer_index] = torch.load(
            os.path.join(load_path, str(layer_index) + ".pth"),
            map_location=model.model.decoder.layers[layer_index].final_layer_norm.weight.device,
        )
        for module in model.model.decoder.layers[layer_index].modules():
