...
class AQEngine(nn.Module):
    ...
    def quantize(self, *, args: Namespace, verbose: bool = True) -> QuantizedWeight:
        ...
        self.quantized_weight = QuantizedWeight(
            ...
            verbose=True,
            multibook_ratio=args.multibook_ratio,
            shuffle_rule=args.shuffle_rule
        )


        differentiable_parameters = nn.ParameterDict(
            {name: param for name, param in self.quantized_weight.named_custom_parameters().items() if param.requires_grad}
        )
    ...
    def beam_search_update_codes_(
        ...
        **kwargs,
        ...
        if len(devices) == 1:  # single device
            ...
            dtype = self.quantized_weight.codebooks[0].dtype
            ...
            assert False, 'parallel is not available now'
