...
def cache_hiddens(model, dataloader, args):
    ...
    cached_hiddens = []
    print(type(dataloader),len(dataloader))
    print(dataloader[0].shape)
    print(dataloader[0])
    print(maybe_get_0th_element(dataloader[0]))
    ...
    for epoch in range(args.epochs):
        ...
        for batch_indices in tqdm(batch_indices_epoch, desc=f"Train epoch {epoch}", leave=False):
            ...
            if not torch.isfinite(loss).item():
                ...
                print(f"Warning: Skipping batch with invalid loss: {loss}")
