...
def get_inps(
    ...
    # if model_seqlen == 2048:
    #     data = data[:(len(data)//2)] # 适配2048的情况
    #     data = torch.cat(data, dim=1) # (1 , 4096*len(data)//2)
...
def quantize_aq(model: PreTrainedModel, data: Sequence, val_data: Optional[Sequence], args: Namespace):
    ...
    for layer_index in range(len(layers)):
        ...
        for names in sequential:
            ...
            # print(layer.self_attn.q_proj.quantized_weight.codebooks.dtype)
...
def perplexity_eval(model: PreTrainedModel, testenc: torch.LongTensor, args: Namespace) -> float:
    ...
    forward_args["attention_mask"] = forward_args["attention_mask"].to(torch.float32)
    layers = get_layers(model)
...
def update_outs(
    ...
    for j in trange(len(inps_tensor), desc="calc outs after quantization", leave=False):
        ...
        # outs_batch = layer(inps_tensor[j].to(torch.float32).to(device).unsqueeze(0), **forward_args)[0]
...
def main():
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=[16],
        help="Each codebook will contain 2 ** nbits_per_codebook vectors for each value in the list",
    )
    parser.add_argument(
        "--multibook_ratio",
        type=float,
        default=1.0,
        help="The ratio of channels allocated to the multi-codebook compared to the total channels",
    )
    parser.add_argument(
        "--shuffle_rule",
        type=int,
        default=3,
        help="The rule to use for re-sorting the channels of weight matrix. 1 for sorting randomly, 2 for sorting by W-only, 3 for sorting by W-A.",
    )
    parser.add_argument(
        "--out_group_size",
        type=int,
    ...
    logging.info(f"Running script with the following parameters: {vars(args)}")
